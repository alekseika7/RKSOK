...
class RKSOKProtocol:
    ...
    def _check_request_data(self) -> None:
        ...
        # if not self._request.value and self._request.command == self.configuration.command_names.put:
        #     raise MissingRequestValueError(
        #         f'Missing value! You must provide a non-empty value for {self._request.command}'
        #     )
    ...
    async def process_request(self, request_data: RequestData) -> str:
        """"""
        self._request = request_data
        try:
            self._check_request_data()
        except RequestCheckBaseException as parsing_error:
            return self.configuration.response_names.incorrect
